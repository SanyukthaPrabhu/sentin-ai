...
nb02 = {
    ...
    "cells": [
        ...
        cell_code([
            ...
            "\n",
            "yolo_weights = str(ROOT / 'models/yolo_custom.pt') if (ROOT / 'models/yolo_custom.pt').exists() else str(ROOT / 'yolov8m-seg.pt')\n",
            "yolo   = YOLOInference(weights=yolo_weights)\n",
        ...
        cell_md([
            "## YOLO → LSTM Interface\n\n",
            "| Feature | Custom Class Mapping | COCO Proxy Mapping (Fallback) | Epidemiological Meaning |\n",
            "|---------|---------------------|-------------------------------|------------------------|\n",
            "| `stagnant_water_count` | `stagnant_water` (Class 0) | `boat/surfboard/frisbee` | Stagnant water site count |\n",
            "| `stagnant_water_area_px` | `stagnant_water` mask pixels | sum of proxy mask areas | Total standing water surface |\n",
            "| `garbage_count` | `garbage_pile` (Class 1) | `suitcase/backpack/bottle/etc` | Waste accumulation count |\n",
            "| `vegetation_anomaly_score` | `vegetation_anomaly` mask density | NDWI timeline deviation | Stressed/dry vegetation stress index |\n\n",
            "> **Production upgrade:** Fine-tune on verified Label Studio datasets. If `models/yolo_custom.pt` is present, the inference pipeline automatically detects it and resolves coordinates using custom classes rather than COCO proxies.",
        ]),
        cell_code([
